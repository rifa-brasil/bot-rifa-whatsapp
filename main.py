...
EVOLUTION_URL = os.environ.get("EVOLUTION_URL", "https://mi-whatsapp-api-pobo.onrender.com")
...
ADMIN_PHONE = os.environ.get("ADMIN_PHONE", "5511948824359") # Tu número de administrador en formato internacional sin '+'
...
async def handle_web(request):
    return web.Response(text="Bot de Rifa con WhatsApp Activo y en Línea 24/7!")
...
def obtener_teclado_idioma():
    return [
        ...
        {"id": "lang_pt", "title": "🇧🇷 Brasileño (Português)"}
    ]
...
def obtener_texto_reglas(lang="es"):
    ...
    if lang == "pt":
        return (
            ...
            f"✨ *Valores para sua primeira jogada:* 1 núm = {VALOR_POR_NUMERO} reais | 5 núm = {int(VALOR_POR_NUMERO * 4)} reais.\n"
            f"⚠️ A partir da segunda jogada, cada número tem custo fixo de {VALOR_POR_NUMERO} reais.\n"
            ...
            "4️⃣ Garantia de reembolso integral com o administrador.\n"
        ...
        return (
            ...
            "4️⃣ Garantía de devolución con el administrador.\n"
...
async def procesar_mensaje_entrante(session, from_phone, mensaje_texto):
        ...
        if comando.startswith("/bloquear"):
            ...
            if len(partes_cmd) > 1 and partes_cmd[1].isdigit():
                ...
                if info_num.get("estado") == "ocupado":
                    ...
                    premio_actual = calcular_premio_total()
                    msg_res = f"🎉 *¡Felicidades al Ganador!* 🎉\n\nEl usuario {ganador_nombre} ha ganado con el número {num_str.zfill(2)} un premio de {premio_actual} reales. ¡Muchas felicidades! 🥳"
                    await enviar_mensaje_whatsapp(session, from_phone, msg_res)
                    if ganador_id:
                        await enviar_mensaje_whatsapp(session, ganador_id, msg_res)
                    return
    ...
    if comando in ["hola", "buenas", "lista", "inicio", "rifa", "sorteo"]:
        ...
        await enviar_mensaje_whatsapp(session, from_phone, texto_resp, interactive_buttons=obtener_teclado_idioma())
        return
    ...
    # Procesamiento de compra de números separados por comas
    ...
    if es_lista_numeros:
        ...
        if estado_actual_rifa in ["finalizada", "bloqueada"]:
            msg_bloq = "⛔ Lo sentimos, la lista se encuentra cerrada o bloqueada." if lang_usuario == "es" else "⛔ Desculpe, a lista está fechada ou bloqueada."
            await enviar_mensaje_whatsapp(session, from_phone, msg_bloq)
            return
...
# --- PROCESADOR DE BOTONES INTERACTIVOS ---
